    ...
    <aside id="sidebar-wrapper">
        ...
        <ul class="sidebar-menu">
            ...
            @foreach (session()->get('single_menu') as $single_menu_item)
                <li class="{{  request()->routeIs($single_menu_item->link) ? 'active' : '' }}"><a class="nav-link " href="{{ url($single_menu_item->link) }}"><i
            ...
            @foreach (session()->get('main_dropdown_menu') as $main_dropdown_menu_item)
                <li class="dropdown active{{  request()->routeIs($main_dropdown_menu_item->link) ? 'active' : '' }}" >
                    <a href="#" class="nav-link has-dropdown"><i class="fas fa-th"></i>
                    ...
                    <ul class="dropdown-menu" style="display: block;" >
                        ...
                        <li class="dropdown">
                            <a class="nav-link pr-0" data-toggle="dropdownini"><span>{{ $submain_dropdown_menu_item->text }}</span>
                            <i class="fa fa-caret-down"></i>
                            </a>
                                <ul class="dropdown-menu" style="display: block;">
                                    @foreach (session()->get('submain_dropdown_menu_item') as $submain_dropdown_menu_item)
                                        <li><a class="{{  request()->routeIs($submain_dropdown_menu_item->link) ? 'active' : '' }}" href="{{ url($submain_dropdown_menu_item->link) }}">{{$submain_dropdown_menu_item->text  }}</a>
                                        </li>                                        
                                    @endforeach
                            ...
                            @foreach (session()->get('dropdown_menu_item') as $dropdown_menu_item)
                                @if($main_dropdown_menu_item->id_menu == substr ($dropdown_menu_item->id_menu, 0, 1))
                                    <li class="{{  request()->routeIs($dropdown_menu_item->link) ? 'active' : '' }}"><a class="nav-link "
